...
const Cta = ({className, language = "hr"}) => {
  ...
  return (
    <Link onClick={() => trackCtaClick("estimateCta")} href={language == "hr" ? "/hr/procjena" : "/en/estimate"} className={`${className} flex w-fit
        text-xl  sm:text-3xl text-white font-semibold bg-primary rounded-xs
        hover:scale-105 transition-transform duration-300 ease-in-out`}
    >
      <span className='w-57 sm:w-83 px-5 py-2  sm:px-6 sm:py-4 text-balance'>{language == "hr" ? "Zatražite Besplatnu Procjenu Zarade" : "Get My Free Earning Estimate"}</span>
      <div className='px-3 py-2  sm:px-6 flex-shrink-0 bg-primary-600 flex items-center justify-center rounded-xs'>
        <RiArrowRightLine className='text-3xl sm:text-4xl' />
      </div>
    </Link>
  )
